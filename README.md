...
The checked-in [`.vscode/settings.json`](.vscode/settings.json) uses
`/usr/bin/clangd-20` and the compilation database generated by the
`clang-debug` preset. Configure that preset at least once before expecting
accurate editor diagnostics:

```bash
cmake --preset clang-debug
```

After installing `clangd-20`, run **Developer: Reload Window** from the VS Code
command palette. To inspect the exact command clangd uses for a file, open
**View: Output** and select **clangd**.
